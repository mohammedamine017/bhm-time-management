...
@Injectable()
export class TimeClockFilesService {
  constructor(private readonly config: ConfigService) {}

  async store(file: UploadedTimeClockFile) {
      ...
      return new Promise<{ storageUrl: string; storageKey: string }>(
        (resolve, reject) => {
          const stream = cloudinary.uploader.upload_stream(
            {
              folder: 'bhm-v2/time-clock-reports',
              resource_type: 'raw',
              use_filename: true,
    ...
    const directory = join(process.cwd(), 'uploads', 'time-clock-reports');
    ...
    return {
      storageUrl: null,
      storageKey: `time-clock-reports/${storedName}`,
    };
  }
}
